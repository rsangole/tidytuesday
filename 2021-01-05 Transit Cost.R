...
sysfonts::font_add(family = "Monoid", regular = "../fonts/Monoid-Retina.ttf")
sysfonts::font_add(family = "Verdana", regular = "../fonts/Verdana.ttf")
...
transit_cost %>%
  mutate(tunnel_per = as.numeric(str_replace(tunnel_per, "%", "")),
         ...
         cc_id = paste(country,city)) -> transit_cost
...
transit_cost %>%
  ...
  filter(total_len<10000,
         total_projects > 1) %>%
  filter(country %in% c("CN", "IN")) %>%
  mutate(cc_id = paste(country,city)) %>%
  mutate(country = ifelse(country == "CN", "China", "India"))  -> dat
...
to_plot %>%
  ...
  scale_discrete_manual(aesthetics = "color",
                        ...
                        guide = F)
ggsave(filename = "ggplot.png")
# embedFonts(file = "ggplot_cm.pdf", fontpaths = "../fonts/")

# transit_cost %>%
#   group_by(country) %>%
#   drop_na() %>%
#   summarise(across(real_cost,
#                    list("mean" = mean,
#                         "max" = max),
#                    .names = "{.col}_{.fn}"
#                    ))
...
#
# lm(
#   real_cost ~ country + year + length + tunnel + tunnel_per + stations,
#   transit_cost
# ) %>% summary()
#
# library(rpart)
# rpart(real_cost ~ country + year + length + tunnel + tunnel_per + stations,
#       transit_cost
#       ) -> mod_part
# mod_part
# plot(mod_part)
